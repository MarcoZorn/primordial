# primordial — live run status

**Run `genesis` · tick 432,000 · day 180 · 6.0 h of compute**

_Updated 2026-09-04 07:43 UTC. Generated from telemetry by `status.py`; nothing here is written by hand._

## Right now

| | |
|---|---|
| organisms alive | **1,800** |
| plants | 965 |
| animals | 835 |
| microbes | 0 |
| species | 24 |
| mean cells per body | 2.31 |
| largest body | 5 cells |
| mean neurons | 40.9 |
| largest brain | 45 neurons |
| deepest lineage | 162 generations |
| oldest living | 70,751 ticks |
| carrion | 661 |
| chirping | 596 |
| total births | 538,584 |
| total deaths | 1,246,253 |
| weather | calm |
| daylight | 1.00 |
| resumes | 2 |

## Cell census

| cell | count | share |
|---|---|---|
| eater | 1,845 | 44.5% |
| photo | 1,026 | 24.7% |
| digest | 691 | 16.7% |
| armor | 411 | 9.9% |
| shell | 127 | 3.1% |
| store | 18 | 0.4% |
| sensor | 14 | 0.3% |
| mover | 9 | 0.2% |
| toxin | 9 | 0.2% |
| core | 0 | 0.0% |

## Milestones reached

| event | tick |
|---|---|
| first photo cell | 1,000 |
| first mover cell | 1,000 |
| first eater cell | 5,000 |
| first sensor cell | 1,000 |
| first toxin cell | 1,000 |
| first 3-cell body | 5,000 |
| first 10-cell body | 163,000 |
| first 25-cell body | not yet |
| first animal | 5,000 |
| 50+ neuron brain | not yet |
| 100+ neuron brain | not yet |

### population

```
              █      █ ████   █ ███████ ████████
              █  █  ███████   █ ███████ ████████
              █  ██ ███████ ████████████████████
              █  ███████████████████████████████
█          █ ███ ███████████████████████████████
██       █ █████████████████████████████████████
```

### mean cells per body

```
                                                
                  █                             
                  ██                            
                 ██████    ████        █        
        ████████████████████████████████████████
     ███████████████████████████████████████████
```

### mean neurons

```
                           █           █        
                   █  █    █████████████████████
                   █████████████████████████████
                 ███████████████████████████████
               █████████████████████████████████
        ████████████████████████████████████████
```

---

There is no fitness function in this simulation. Nothing above was a target; it is what survived.

Source: [`genesis/telemetry.jsonl`](runs/genesis/telemetry.jsonl)
